...
getLabels n = nextState cmd
    where cmd = defCmd{cmdMethod = HT.methodGet, cmdPath = path, cmdBody = "", cmdParse = parser}
          ...
          parser jn = G.setNodeLabels (NodeUrl $ tryParseFrom jn) (tryParseBody jn)
...
addLabels lbls n = nextState cmd
    where cmd = defCmd{cmdMethod = HT.methodPost, cmdPath = path, cmdBody = "", cmdParse = parser}
          path = getNodeBatchId n <> "/labels"
          parser jn g = let from = (NodeUrl $ tryParseFrom jn) in foldl (flip (G.addNodeLabel from)) g lbls

-- | Change node labels
-- | Raises Neo4jNoEntityException if the node doesn't exist
changeLabels :: NodeBatchIdentifier a => [Label] -> a -> Batch (BatchFuture ())
changeLabels lbls n = nextState cmd
    where cmd = defCmd{cmdMethod = HT.methodPut, cmdPath = path, cmdBody = "", cmdParse = parser}
          path = getNodeBatchId n <> "/labels"
          parser jn = G.setNodeLabels (NodeUrl $ tryParseFrom jn) lbls
...
removeLabel lbl n = nextState cmd
    where cmd = defCmd{cmdMethod = HT.methodPut, cmdPath = path, cmdBody = "", cmdParse = parser}
          path = getNodeBatchId n <> "/labels/" <> lbl
          parser jn = G.deleteNodeLabel (NodeUrl $ tryParseFrom jn) lbl
